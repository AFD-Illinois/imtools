# Ring Extractor fitting code
# Barely adapted from original, original license preserved
...
RPRIOR_MIN = 15.   # minimum radius for search (muas)
...
NORMFLUX = 0.7    # normalized image flux for outputted profiles (Jy)
...
def FindProfileSingle(im, out_dir = './', out='prof', save_files=False, blur=0, aipscc=False, tag='', rerun=True,return_pp=True):
    ...
    # im = eh.image.load_image(imname)
    imname = out
    # txtname = POSTPROCDIR + '/' + dirname + '/' + basename[:-5] +tag+ '.txt'
    txtname = out_dir+out+tag+'.txt'
    ...
    with HiddenPrints():
        ...
        if save_files:
            ...
            txtname = out_dir+ out+tag+'.txt'
            radprof_name = out_dir+out+tag+'_radprof.txt'
            angprof_name = out_dir+out+tag+'_angprof.txt'
            ...
            fitsname = out_dir + out+tag+'.fits'
            fitsname_centered = out_dir+out+tag+'_cent.fits'
